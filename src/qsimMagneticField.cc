...
qsimMagneticField::qsimMagneticField( G4String filename ){ 
...
qsimMagneticField::~qsimMagneticField(){ 
}

G4String qsimMagneticField::GetName(){ 
    if( !fInit ){
...
void qsimMagneticField::SetFieldScale(G4double s){ 
    fFieldScale = s;
    G4cout << fFilename << " scale set to " << s << G4endl;
    return;
}

void qsimMagneticField::SetMagnetCurrent(G4double s){ 
    if( fMagCurrent0 > 0.0 ){
...
void qsimMagneticField::InitializeGrid() {
    /*!  
...
void qsimMagneticField::ReadFieldMap(){
...
void qsimMagneticField::GetFieldValue(const G4double Point[4], G4double *Bfield ) const {
