...
qsimGlobalField::qsimGlobalField(){
}

qsimGlobalField::~qsimGlobalField(){
}

void qsimGlobalField::AddNewField( G4String name ){

    qsimMagneticField *thisfield = new qsimMagneticField(name);
...
/*    if( thisfield->IsInit() ){
	...
	qsimRunData *rd = qsimRun::GetRun()->GetData();
...
qsimMagneticField *qsimGlobalField::GetFieldByName(G4String name) {
    std::vector<qsimMagneticField*>::iterator it = fFields.begin();
...
void qsimGlobalField::GetFieldValue( const G4double p[], G4double *resB) const {
    ...
    std::vector<qsimMagneticField*>::const_iterator it = fFields.begin();
...
void qsimGlobalField::SetFieldScale( G4String name, G4double scale ){
	qsimMagneticField *field = GetFieldByName(name);
	if( field ){
...
void qsimGlobalField::SetMagnetCurrent( G4String name, G4double scale ){
	qsimMagneticField *field = GetFieldByName(name);
	if( field ){
